...
from yolo_tracking import TrajectoryDatasetRecorder, YoloSingleObjectTracker


APP_TITLE = "Orbbec Astra+ | RGB + Profundidad + IR"
PANEL_SIZE = (640, 480)
...
class CameraConfig:
    ...
    yolo_target_class: str = "senalamiento_trafico"
    yolo_traffic_only: bool = True
    yolo_confidence: float = 0.02
    yolo_iou: float = 0.50
    yolo_device: str = "cpu"
    yolo_tracker_config: str = "bytetrack_stop.yaml"
    yolo_dataset_dir: str = "evidencias_pruebas/senalamiento_trafico"
...
def telemetry_panel(
    ...
    cv2.putText(
        panel,
        "1 RGB / 2 PROF / 3 IR | A/Z bajo +/- | S/X alto +/-",
        (18, 78),
        cv2.FONT_HERSHEY_SIMPLEX,
        0.46,
        (190, 190, 190),
    ...
    cv2.putText(
        panel,
        "Deteccion exclusiva: YOLO | clase 0 senalamiento_trafico",
        (18, 99),
        cv2.FONT_HERSHEY_SIMPLEX,
        0.43,
        (120, 230, 170),
    ...
    cv2.putText(
        panel,
        "Un solo objetivo | bloqueo por Track ID | captura automatica",
        (18, 118),
        cv2.FONT_HERSHEY_SIMPLEX,
        0.39,
        (190, 190, 190),
    ...
    cv2.putText(
        panel,
        "Detectores por color y forma: AISLADOS / INACTIVOS",
        (18, 137),
        cv2.FONT_HERSHEY_SIMPLEX,
        0.37,
        (190, 190, 190),
    ...
    yolo_text = f"YOLO ALTO: AUTO | {trajectory_recorder.status}"
    ...
    sensor_names = ["infrarrojo", "profundidad", "rgb"]
...
def save_raw_capture(
    ...
    cv2.imwrite(str(CALIBRATION_DIR / f"ir_{stamp}.png"), ir)
    return destination
...
def make_telemetry(
    ...
    ir_min, ir_max, ir_mean = stats(ir)
    ...
    return {
        "infrarrojo": [
            f"Resolucion: {ir.shape[1]}x{ir.shape[0]}",
            f"FPS: {fps['ir']:.1f}",
            pixel_line("ir"),
            edge_line("ir"),
            f"Min / max / media: {ir_min:.0f} / {ir_max:.0f} / {ir_mean:.1f}",
        ],
        "objetivo": objective,
        "profundidad": [
...
def trajectory_sample_metadata(
    ...
    return {
        ...
        "ir_resolution": [ir.shape[1], ir.shape[0]],
        "fps": fps,
        ...
        "ir_active_pixels": int(np.count_nonzero(ir)),
        "ir_min_raw": ir_min,
        "ir_max_raw": ir_max,
        "ir_mean_raw": ir_mean,
        "depth_scale_mm": config.depth_scale_mm,
...
def main() -> None:
    ...
    if config.yolo_traffic_only and config.yolo_model == "yolo11n.pt":
        config.yolo_model = "yolov8s-world.pt"
        config_changed = True
    if config.yolo_traffic_only and config.yolo_target_class != "senalamiento_trafico":
        config.yolo_target_class = "senalamiento_trafico"
        config_changed = True
    if config.yolo_dataset_dir == "yolo_dataset":
        config.yolo_dataset_dir = "evidencias_pruebas/senalamiento_trafico"
        config_changed = True
    if config.yolo_traffic_only and config.yolo_confidence > 0.05:
        config.yolo_confidence = 0.02
        config_changed = True
    if config_changed:
        config.save(CONFIG_PATH)
        print("Configuracion YOLO migrada a una sola clase: senalamiento_trafico.")
    ...
    settings = DetectionSettings(mode="yolo", target="senalamiento_trafico")
    ...
    ir_filter = TemporalKalman(process_noise=1.0, measurement_noise=40.0)
    edge_settings = EdgeSettings()
    alignment_settings = AlignmentSettings.from_config(config)
    capture_options = CaptureOptions()
    fps_counters = {name: FpsCounter() for name in ("rgb", "depth", "ir")}
    screenshot_count = 0
    ...
    try:
        ...
        while True:
            ...
            ir_timestamp_ns = time.time_ns()
            ir_raw = np.frombuffer(
                ir_frame.get_buffer_as_uint16(), dtype=np.uint16
            ).reshape(ir_frame.height, ir_frame.width).copy()
            ...
            ir_smoothed = ir_filter.update(ir_raw)
            ...
            if yolo_tracker is None and yolo_load_error is None:
                try:
                    ...
                    print(
                        "YOLO exclusivo listo. Clase unica: "
            ...
            detection = (
                find_yolo_target(yolo_rgb, yolo_tracker)
                if yolo_tracker is not None
                else None
            )
            if detection is not None and config.rgb_flip_horizontal:
                mirror_detection_horizontal(detection, rgb.shape[1])
            if detection:
                tracked_center = detection.center
                lost_frames = 0
                measure_detection(detection, rgb.shape, aligned_depth_raw, config)
            else:
                lost_frames += 1
                if lost_frames > 15:
                    tracked_center = None
            ...
            if config.trajectory_auto_capture:
                if detection is not None and detection.source == "yolo":
                    ...
                    if (
                        ...
                        recording_missing_frames = 0
                        print(
                            "ALTO detectado: trayectoria automatica iniciada: "
                            f"{directory}"
                        )
                else:
                ...
                if trajectory_recorder.active and recording_missing_frames > 30:
                    ...
                    recording_missing_frames = 0
                    print("Trayectoria automatica cancelada: se perdio el señalamiento.")
            ...
            ir_u8 = normalize_u8(ir_smoothed)
            ir_low, ir_high = edge_settings.thresholds("ir")
            ir_panel, ir_edges = canny_edges_only(
                ir_u8, ir_low, ir_high, (0, 255, 0)
            )

            if detection:
                detection_label = detection_display_label(detection)
                draw_detection(rgb_panel, detection.bbox, detection_label, (0, 220, 255))
                ir_bbox = map_bbox(detection.bbox, rgb.shape, ir_panel.shape)
                draw_detection(overlay_panel, detection.bbox, detection_label, (0, 220, 255))
                draw_detection(ir_panel, ir_bbox, "ROI RGB*", (0, 220, 255))

            panels = []
            for panel, title in (
                (rgb_panel, "RGB + Kalman + Canny"),
                (overlay_panel, "ENSAMBLE RGB + PROFUNDIDAD"),
                (ir_panel, "INFRARROJO + Kalman + Canny"),
            ):
            ...
            if key == ord("3"):
                edge_settings.selected_sensor = "ir"
